...
When using Kubernetes it is considered good practice to limit API server access as much as possible and when using a cloud managed kubernetes offering, you are not by default in control of the networks used by kubernetes. The default configuration by kubernetes exposes the API server on the public internet to provide you access to your cluster and this is fine when you're giving it a try but once you start using kubernetes more seriously you will probably want to start locking down access. Most cloud providers provide mechanisms to limit access with IP whitelisting and [private clusters](https://docs.microsoft.com/en-us/azure/aks/private-clusters) (API server has a private instead of a public IP).
